      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          if [ 0 -eq $rc -a x"$revlist" != x ]; then
            ...
            set +e
            docker images | grep basal-middleware | grep $tag
            rc=$?
            set -e
            if [ 0 -eq $rc ]; then
              DOCKER_REGISTRY=$DOCKER_REGISTRY make release-docker-images
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          sed -i "s/replicas: 2/replicas: $REPLICAS_COUNT/g" cmd/basal-middleware/k8s/02-basal-middleware.yaml
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          sed -i "s/replicas: 2/replicas: $REPLICAS_COUNT/g" cmd/basal-middleware/k8s/02-basal-middleware.yaml
          sed -i "s/imagePullPolicy: Always/imagePullPolicy: IfNotPresent/g" cmd/basal-middleware/k8s/02-basal-middleware.yaml
          make deploy-to-k8s-cluster
        '''.stripIndent())
      }
    }
